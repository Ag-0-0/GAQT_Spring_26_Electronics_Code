...
const int number_of_readings = 12;
...
void setup_ozone_two() {
  Serial.begin(9600); // I believe this is the baud rate. In the ozone only code, this worked perfectly, but here it may need to be adjusted to whatever the current baud rate is.
...
void setup_readings() { // TODO
  readings[0].upperbound = 1000;  // Mass Concentration Pm1p0
  readings[0].lowerbound = 0;  // Mass Concentration Pm1p0
  readings[1].upperbound = 1500;  // Mass Concentration Pm2p5
  readings[1].lowerbound = 5;  // Mass Concentration Pm2p5
  readings[2].upperbound = 1000000;  // Mass Concentration Pm4p0
  readings[2].lowerbound = 1000;  // Mass Concentration Pm4p0
  readings[3].upperbound = 2000;  // Mass Concentration Pm10p0
  readings[3].lowerbound = 10;  // Mass Concentration Pm10p0
  readings[4].upperbound = 1000000;  // Ambient Humidity
  readings[4].lowerbound = 1000;  // Ambient Humidity
  readings[5].upperbound = 2400;  // Ambient Temperature
  readings[5].lowerbound = 22;  // Ambient Temperature
  readings[6].upperbound = 15000;  // VOC Index
  readings[6].lowerbound = 100;  // VOC Index
  readings[7].upperbound = 15000;  // nox Index
  readings[7].lowerbound = 100;  // nox Index
  readings[8].upperbound = 200;  // Carbon Dioxide
  readings[8].lowerbound = 100;  // Carbon Dioxide
  readings[9].upperbound = 3000;  // Temperature
  readings[9].lowerbound = 22;  // Temperature
  readings[10].upperbound = 1000000; // Humidity
  readings[10].lowerbound = 1000; // Humidity
  readings[11].upperbound = 100; // Ozone
  readings[11].lowerbound = 5; // Ozone
}
...
void read_ozone() {
  ...
  readings[11].value = Ozone.readOzoneData(COLLECT_NUMBER); //in PPB. 
  // the prior line may throw an error due to a type mismatch, as value is a float but the reading is in int16_t. In this case, try casting or other debugging methods.
}
...
void printer() {
  // particulate matter in units of micrograms per meter cubed
  Serial.print("Pm1p0:   ");
  Serial.println(readings[0].value);
  Serial.print("Pm2p5:   ");
  Serial.println(readings[1].value);
  Serial.print("Pm4p0:   ");
  Serial.println(readings[2].value);
  Serial.print("Pm10p0:   ");
  Serial.println(readings[3].value);
  Serial.print("Humidity from Sen55:   ");
  Serial.println(readings[4].value);
  Serial.print("Temp from Sen55:   ");
  Serial.println(readings[5].value);
  Serial.print("VOC:   ");
  Serial.println(readings[6].value);
  Serial.print("NOX:   ");
  Serial.println(readings[7].value);
  Serial.print("CO2:   ");
  Serial.println(readings[8].value);
  Serial.print("Temp from Ozone:   ");
  Serial.println(readings[9].value);
  Serial.print("Humidity from Ozone:   ");
  Serial.println(readings[10].value);
  Serial.print("Ozone");
  Serial.println(readings[11].value);
}

void loop() {
  while (n<100) { //n controls the number of readings.
  read_sen55();
  read_ozone();
  update_isactives();
  fan_control(12);
